  ...
  <div class="container" :class="{ container__dark: isDarkMode }">
    ...
    <div class="recover">
      <div class="recover--wrapper">
        ...
        <h4
          class="recover--title"
          :class="{ 'recover--title__dark': isDarkMode }"
        >
          Recover Account
        </h4>
        <form @submit.prevent="recoverHandler">
          <input
            type="email"
            placeholder="design.code@designcode.com"
            class="recover--input"
            :class="{ 'recover--input__dark': isDarkMode }"
            v-model="email"
            required
          />
          <button class="recover--button">Send Email</button>
        </form>
        <router-link
          to="/signin"
          class="recover--link"
          :class="{ 'recover--link__dark': isDarkMode }"
        >
